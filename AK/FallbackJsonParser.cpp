...
ErrorOr<JsonValue> FallbackJsonParser::parse(StringView input)
{
    FallbackJsonParser parser(input);
...
//                             ╰─── u[0-9A-Za-z]{4}  ──╯
//
ErrorOr<ByteString> FallbackJsonParser::consume_and_unescape_string()
{
    if (!consume_specific('"'))
...
ErrorOr<JsonValue> FallbackJsonParser::parse_object()
{
    JsonObject object;
    if (!consume_specific('{'))
        return Error::from_string_literal("JsonParser: Expected '{'");
...
ErrorOr<JsonValue> FallbackJsonParser::parse_array()
{
    JsonArray array;
    if (!consume_specific('['))
        return Error::from_string_literal("JsonParser: Expected '['");
...
ErrorOr<JsonValue> FallbackJsonParser::parse_string()
{
    auto string = TRY(consume_and_unescape_string());
    return JsonValue(move(string));
}

ErrorOr<JsonValue> FallbackJsonParser::parse_number()
{
...
ErrorOr<JsonValue> FallbackJsonParser::parse_true()
{
...
ErrorOr<JsonValue> FallbackJsonParser::parse_false()
{
...
ErrorOr<JsonValue> FallbackJsonParser::parse_null()
{
...
ErrorOr<JsonValue> FallbackJsonParser::parse_helper()
{
    ignore_while(is_space);
    auto type_hint = peek();
    switch (type_hint) {
...
ErrorOr<JsonValue> FallbackJsonParser::parse_json()
{
