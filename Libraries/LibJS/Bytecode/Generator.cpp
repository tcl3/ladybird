...
void Generator::emit_put_by_value(ScopedOperand const& base, ScopedOperand property, ScopedOperand const& src, Bytecode::Op::PropertyKind kind, Optional<IdentifierTableIndex> const& base_identifier)
{
    if (property.operand().is_constant() && get_constant(property).is_string()) {
        auto property_key = MUST(get_constant(property).to_property_key(vm()));
        if (property_key.is_string()) {
...
void Generator::emit_put_by_value_with_this(ScopedOperand const& base, ScopedOperand property, ScopedOperand const& this_value, ScopedOperand const& src, Bytecode::Op::PropertyKind kind)
{
    if (property.operand().is_constant() && get_constant(property).is_string()) {
        auto property_key = MUST(get_constant(property).to_property_key(vm()));
        if (property_key.is_string()) {
