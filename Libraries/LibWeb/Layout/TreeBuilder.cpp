...
static Optional<FirstLetterTarget> find_first_letter_in_block(BlockContainer& block)
{
    ...
    for (auto* child = block.first_child(); child; child = child->next_sibling()) {
        ...
        if (!inner_block)
            break;
        // If the descendant block has its own ::first-letter, it takes precedence over the ancestor's.
        if (auto* dom_element = as_if<DOM::Element>(inner_block->dom_node());
            dom_element && dom_element->computed_properties(CSS::PseudoElement::FirstLetter))
            break;
...
void TreeBuilder::create_first_letter_wrapper_if_needed(DOM::Element& element, BlockContainer& block_container)
{
    ...
    first_letter_slice->set_generated_for(CSS::PseudoElement::FirstLetter, element);
