...
void PaintableBox::visit_edges(Cell::Visitor& visitor)
{
    ...
    for (auto const& column_background : m_table_column_backgrounds) {
        visitor.visit(column_background.column.layout_box);
        visitor.visit(column_background.column_group.layout_box);
    }
}
