...
void paint_background(DisplayListRecordingContext& context, PaintableBox const& paintable_box, CSS::ImageRendering image_rendering, ResolvedBackground const& resolved_background, BorderRadiiData const& border_radii)
{
    ...
    if (resolved_background.paint_clip_box.has_value()) {
        ...
        display_list_recorder.add_clip_rect(clip_rect.to_type<int>());
        ScopedCornerRadiusClip corner_clip { context, clip_rect, clip_box.radii, CornerClip::Outside, !is_root_element };
...
{
    return resolve_background_layers(layers, paintable_box.layout_node_with_style_and_box_metrics(), paintable_box.box_model(), background_color, background_color_clip, border_rect, border_radii);
}
